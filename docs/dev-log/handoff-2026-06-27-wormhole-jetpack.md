...
- 虫洞期间暂停普通移动、互动、喷气背包、碰撞任务触发、氧气消耗和背景音乐，保持寂静。
...
- 虫洞中 `W/A/S/D` 和方向键控制轻微画面漂移，已修正为屏幕同向移动，并加入更明显的阻尼、回弹和边界阻力，松开自动回中；第三人称虫洞镜头已下压并把观察点抬到宇航员躯干中心，让人物默认更靠近屏幕/虫洞正中；`C` 仍可切换第一/第三人称。
- 飞行背包拆成两种：
